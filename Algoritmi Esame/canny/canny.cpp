...
void Canny(const Mat src, Mat& dst){ // passo la matrice sorgente e la matrice destinazione
    ...
    for (int y = 1; y < magnitude.rows-1; y++){
        for(int x=1; x<magnitude.cols-1; x++){
            ...
            } else if((angle< -157.5 || angle > 112.5) || (angle >= -67.5 && angle <= 22.5)){ // controllo se il gradiente è diagonale 45
                q = magnitude.at<uchar>(y+1,x-1);
                r = magnitude.at<uchar>(y-1,x+1);
            } else if((angle< -112.5 || angle > 67.5) || (angle >= -112.5 && angle <= 67.5)){ // controllo se il gradiente è verticale
                q = magnitude.at<uchar>(y+1,x);
                r = magnitude.at<uchar>(y-1,x);
            } else if((angle< -67.5 || angle > 22.5) || (angle >= -157.5 && angle <= 112.5)){ // controllo se il gradiente è diagonale -45
                q = magnitude.at<uchar>(y-1,x-1);
                r = magnitude.at<uchar>(y+1,x+1);
            }
            if (mag < r || mag < q)
        ...
        for( int i=1; i<magnitude.rows-1;i++){ // applico il thresholding
            for( int j = 1; j<magnitude.cols-1; j++){
                ...
                if(px >= th2) px = 255; // se il pixel è maggiore del threshold alto lo imposto a 255
                else if (px < th1) px = 0; // se il pixel è minore del threshold basso lo imposto a 0
                else{
